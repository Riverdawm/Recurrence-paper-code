...
class Ds_Lpa():
    ...
    def cal_boundary_value(self, G, allnodeslist, final_nodes):
        ...
        temp = 0
        for i in allnodeslist:
            final_nodes[temp].density_star = (final_nodes[temp].density - self.nodes_density_min)/(self.nodes_density_max - self.nodes_density_min)
            final_nodes[temp].distance_star = (final_nodes[temp].distance - self.nodes_distance_min)/(self.nodes_distance_max - self.nodes_distance_min)
...
if __name__ == '__main__':
    ...
    # 导入txt文件绘制成相应无向图
    G = Graph().createGraph('google.txt')
    # nx.draw(G)
    # plt.show()
